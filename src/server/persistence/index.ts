...
export class SqlRunStore implements RunStore {
  ...
  playerSync(rec: PlayerRecord): void {
    this.sql.exec(
      `INSERT INTO player_record (player_id, name, alive, cls, profile, abilities, base, inv, gold, char_xp, chosen_class, talents, talent_points, attr_points, auto_attack, last_seen) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
       ON CONFLICT(player_id) DO UPDATE SET name=excluded.name, alive=excluded.alive, cls=excluded.cls,
         ...
         chosen_class=excluded.chosen_class, talents=excluded.talents, talent_points=excluded.talent_points, attr_points=excluded.attr_points,
         auto_attack=excluded.auto_attack, last_seen=excluded.last_seen`,
      rec.playerId,
      rec.name,
      rec.alive ? 1 : 0,
      rec.cls,
  ...
  async loadPlayer(playerId: string): Promise<PlayerRecord | null> {
    const rows = this.sql
      .exec<PlayerRow>("SELECT player_id, name, alive, cls, profile, abilities, base, inv, gold, char_xp, chosen_class, talents, talent_points, attr_points, auto_attack, last_seen FROM player_record WHERE player_id = ?", playerId)
      .toArray();
